...
class ConfidenceMaskEstimator(nn.Module):

    def __init__(self, config, input_size):
        super().__init__()
        self.config = config
        self.layer1 = nn.Conv2d(config.hidden_size, config.hidden_size, 1)
        self.layer2 = nn.Conv2d(config.hidden_size, config.frame_shape[0], 1)
        # self.relu = nn.ReLU(inplace=False)
        # self.dense1 = nn.Linear(input_size, 256)
        # self.dense2 = nn.Linear(256, 256)
        # self.dense3 = nn.Linear(256, input_size)
...
class NextFramePredictor(Container):
    ...
    def __init__(self, config, n_action):
        ...
        self.conf_estimator = ConfidenceEstimator(self.config, middle_shape[0] + filters)
        self.conf_mask_estimator = ConfidenceMaskEstimator(self.config, middle_shape[0] + filters)
    ...
    def forward(self, x, action, target=None, epsilon=0):
        ...
        confidence_pred = self.conf_estimator(torch.cat((x_mid, x_fin), dim=1))
        confidence_mask_pred = self.conf_mask_estimator(x)
        confidence_pred = F.sigmoid(confidence_pred)
        # Can be softmax later
        confidence_mask_pred = F.sigmoid(confidence_mask_pred)
        x = self.logits(x)
        x = x.view((-1, 256, *self.config.frame_shape))
        return x, reward_pred, value_pred, confidence_pred, confidence_mask_pred
